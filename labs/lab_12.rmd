...
md_zip_code_and_pop <- md_zip_population %>%
  left_join(maryland_zip, by = c("zip_code" = "GEOID")
            
            
md_zip_code_and_pop <- md_zip_code_and_pop %>%
  mutate(percentage_fully_vax = fully_vaccinated/population*100000)
...
ggplot() + 
  geom_sf(data=less_fifty_percent, aes(fill=less_fifty_percent)) + 
  theme_minimal() +
...
ggplot() + 
  ...
  geom_sf(data=less_fifty_percent) +
  theme_minimal()
