...
**Answer** We get a dataframe with tweets from politicians - name and area of representation - using the words either coronavirus or covid beginning Feb. 1, 2020.
...
**Answer** It gives us just one example of each unique thing. This helps us analyze each unique word.
...
**Answer** It gives us an output of all the unique words in our first dataset, which is 5 million unique words. 
...
**Answer** It shows us the top 25 words in graph form used in all of the tweets we compiled, many of which are generic words. The exceptions are covid related.
...
**Answer** We get a list of words and how often they appear that is more targeted to the topic (covid) at hand. We took out all the stop words. This dataset is much more unique than the first one we created.
...
**Answer** We get two dataframes that give us the top 10 unique words in these tweets in 2020 and 2021. This allows us to see which words have changed as the pandemic evolved.
...
**Task** Run the following code and describe the results.
**Answer** It gives us a dataframe with the top 10 bigrams - two words found back-to-back - from our list of tweets in 2020. Unsurprisingly, most of the terminology is covid-related. 
...
**Task** Run the following code and describe the results.
**Answer** It gives us a dataframe with the top 10 bigrams from our list of tweets in 2021. It looks like health care and public health have slipped, while vaccine and relief have become more frequent. This tells us that as vaccines became more readily available, politicians started talking about them at a commensurate rate.
...
**Task** Run the following code and describe the results.
**Answer** We get two dataframes showing us the top 10 bigrams for Democrats and Republicans. There are more tweets on the Democratic side, but both sides seem to be talking about covid in similar ways (resepctfully), at least early in the pandemic. It seems like both sides had similar thought processes in early 2020.
...
**Task** Run the following code and describe the results.
**Answer** This gives us a dataframe with the top 10 bigrams used by Republicans in 2020 and 2021. Trump goes away, but that probably has to do with him leaving office. Social distancing goes away in 2021, which makes sense, but god bless drops off, which is a bit surprising - religion seems to be important to many politicians. Words like illegal immigrants and vaccine mandates and Joe Biden appear.
...
**Answer** We get two dataframes (2020) and (2021) showing the sentiment of words using the function "get_sentiments." This is a cool function, but we run into the issue of certain words having both negative and positive connotations. For example, NEGATIVE most likely would be a negative word, but if someone is covid negative, that is a good thing. The same can be said for the word POSITIVE. 
