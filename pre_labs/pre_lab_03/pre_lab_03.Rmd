...
**Answer** There were 87,927 problems.
...
**Answer** No, it shows zero errors!
...
**Answer** The data type for "amount" is "<dbl>," double or number. The data type for "name" is "<chr>," character. The data type for "id" is also "<dbl>".
```{r}
...
**Answer**: For NA: 192959 loans. For "Subway": 275
...
**Answer**: 135073. Yes, this answer is the same as what we got in Task 8.
...
**Answer**: 135073. Yes, the answer is the same as the previous two codeblocks. What I conclude from the answer is that there are multiple ways to reach the desired end result. The only difference is that certain pieces of code will make it easier to sort through the data. For example, if we had a massive data set, the code we wrote in this final block would be more useful in narrowing down what we want.
...
**Answer** There are no loans in the data for October 2020. For 2020, we have data from April to August.
```{r}
ppp_maryland_loans %>%
...
**Answer** Largest: 10000000. This isn't suspicious because the maximum loan a business could apply for was 10 million dollars. Smallest: 6. This isn't as much suspicious as it is odd. Who would spend the time to fill out the form for a 6 dollar loan? I'm just speculating here, but this might have been a clerical error. For example, the business might have received 600 dollars. If that is the case, however, it raises some serious questions about the tracking of funds with regards to the program.
```{r}
...
**Answer** Yes, all four amount categories align: 78 for LEGACY SPINE AND PAIN LLC and 6 for GETGFTD LLC.
...
**Answer** 
1. The column headers are inconsistently styled. The first column “1_id” starts with a number. The “NAME” column is all caps, while the rest are lowercase. And “street address” has a space in it. Those problems will make them hard to analyze, to refer to in functions we write.
2. The street address field is inconsistent. It has multiple variations of Ritchie Hwy.
3. The city field has five different forms – including misspellings – of Arnold. If we wanted to group and count the number of loans in Arnold, this inconsistency would not let us do that correctly.
...
**Answer** It puts a lowercase x in front of "1_id," making it "x1_id".
...
**Answer**We get the following error message: "Error in 'h()'. It seems like the issue has to do with the following code: "summarise(total_amount = sum(amount))." I think the issue has to do with the placement of the information within the alignment of the actual column in the dataset.
...
**Answer** There are two duplicate rows. The business name is "STUART JONES LLC."
...
**Answer** This standarizes a specific field, "ZIP codes" in this case.
...
**Answer** 2 misspellings of Arnold remain after we run this code.
```{r}
# cleaning function
cleaned_arnold_md_loans <- arnold_md_loans %>%
  clean_names() %>%
...
**Answer** One misspelling remains.
...
**Answer** No misspellings are left! 
```{r}
# cleaning function
cleaned_arnold_md_loans <- arnold_md_loans %>%
  clean_names() %>%
