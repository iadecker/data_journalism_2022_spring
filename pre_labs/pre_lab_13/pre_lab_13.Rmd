...
**Answer** It shows the top states that received the percent of the state's small business that received a loan compared with how that state voted in 2016.
```{r}
...
**Answer** We see a graph that shows there does see to be some correlation between small businesses in states that voted more strongly for Trump getting loans at higher rates than small businesses in Democratic states. It's not the steepest line, but it is more than a horizontal slope. 
```{r}
...
### Task 4: Plot data
**Task** Run the following code. Describe the plot. What do you see?
**Answer** While there does still seem to be a relationship between trump states and the average amount of money provided by PPP per small business employee in the state, it is less pronounced than the previous graph, which means that there is even less correlation with this assertion than thre previous one.
```{r}
...
**Answer** We see that the ppe_amount_per_employee by pct_trump is .22 on the -1 to 1 scale, which shows some correlation but not a lot. In the businesses_receiving_ppe_pct by pct_trump, we see the correlation coefficient is .52, showing a bit more correlation. In other words, the correlation coefficient here is showing us numerically what the graph showed us visually.
```{r}
reveal_data %>%
...
**Answer** It shows us the p-value of 0.00008607, which is quite low. This means that the chance that this is a statistical aberration, what the p-value measures, of our data is very low. We also see again the the correlation coefficient r value of 0.5218038 
```{r}
...
**Task** Run the following code. Describe the model output. What do you see?
**Answer** Here, the p-value is 0.1189, meaning that it is more likely that our conclusion is statistical aberration. We also see again the correlation coefficient r value of 0.221133 
```{r}
...
**Answer** We see that for both mean_ppp_amount_per_employee and mean_businesses_receiving_ppe_pct, states that went trump had higher values. 
...
**Answer** We see that the t-value is 1.2223 and that the p-value is 0.2295. Both of these numbers indicate that the questions we are asking ourselves could be of significance.
```{r}
...
### Task 10: Run t-test
**Task** Run the following code. Describe the output. What do you see?
**Answer** Here, our t-value is 3.182 (higher than the last bit) and our p-value (0.002643) is quite low. In other words, this tells us that we definitely are honing in on something significant.
```{r}
