...
census_naics_ohio <- ohio_cbp %>%
  filter(naics == "722513") %>% 
  group_by(fips) %>% 
  summarise(businesses = sum(est)) %>%
  arrange(desc(businesses))

ohio_counties>plus_fips <- ohio_primary_naics_code %>%
